...
def sdc_conv_block(end_points,
    ...
    for i in range(sdc_num):
        ...
        if i + 1 < sdc_num:
            ...
            end_points[scope_with_sdc(scope_dconv + DCONV, i + 1)] = net_d
...
def sdc_fully_connected_block(end_points,
    ...
    for i in range(sdc_num):
        ...
        if i + 1 < sdc_num:
            ...
            end_points[scope_with_sdc(scope_dfully_connected + DCONV, i + 1)] = net_d
        net = is_reuse(net, net_d, i, sdc_num)
    ...
    for layer_name in layers_scope:
        ...
        for end_point in end_points:
            if end_point.startswith(layer_name):
                num_layer_losses += 1
        if num_layer_losses <= 1:
            ValueError('Error setting loss value: odd number of losses: %d' % num_layer_losses)
            return 1

        # Find something like: 'conv1_sdc_0' and 'conv1_sdc_1'
        for i in range(sdc_num):
            input_scope = scope_with_sdc(layer_name, i)
            output_scope = scope_with_sdc(layer_name, i + 1)
            find = False
            for end_point in end_points:
                if end_point == output_scope:
                    if end_points[input_scope] is not None:
                        model_losses[index] = add_loss(end_points[input_scope], end_points[output_scope])
                        find = True
                        break
                    else:
                        ValueError(input_scope + ' is not found in "end_points"')
                        return 1
            # Find something like: 'conv1_sdc_0' and 'conv1_dconv2d_sdc_1'
            if not find:
                output_scope = scope_with_sdc(layer_name + DCONV, i + 1)
                for end_point in end_points:
                    if end_point == output_scope:
                        if end_points[input_scope] is not None:
                            model_losses[index] = add_loss(end_points[input_scope], end_points[output_scope])
                            ...
                            return 1
            if not find:
                ValueError(output_scope + ' is not found in "end_points"')
                return 1
