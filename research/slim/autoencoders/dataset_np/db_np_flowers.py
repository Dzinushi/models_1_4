...
class DatabaseFlowers(DatabaseNumpy):
    ...
    def create_table(self):
        super(DatabaseFlowers, self).create_table(Table.flowers_train, {'image': 'text'})
        super(DatabaseFlowers, self).create_table(Table.flowers_validate, {'image': 'text'})

    def select_batch_by_index(self, table, index, batch_size):
        return super(DatabaseFlowers, self).select_batch_by_index(table, 'image', index, batch_size)

    def insert(self, table, id, image_np):
        if table == Table.flowers_train or table == Table.flowers_validate:
            super(DatabaseFlowers, self).insert(table=str(table), id=id, column_name='image', image_np=image_np)
        else:
