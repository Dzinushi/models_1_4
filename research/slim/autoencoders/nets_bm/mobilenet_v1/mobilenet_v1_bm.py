        ...
        with slim.arg_scope([slim.conv2d, slim.separable_conv2d], padding='SAME'):
            ...
            for i, conv_def in enumerate(conv_defs):
                ...
                if isinstance(conv_def, Conv):
                    ...
                    for j in range(sdc_num):
                        end_points[end_point] = net = slim.conv2d(net, depth(conv_def.depth), conv_def.kernel,
                                                                  stride=conv_def.stride,
                                                                  normalizer_fn=slim.batch_norm,
                                                                  reuse=reuse,
                                                                  scope=end_point)
                        end_points[end_point + deconv] = net_d = slim.conv2d_transpose(net, channel, conv_def.kernel,
                                                                                       stride=conv_def.stride,
                                                                                       normalizer_fn=slim.batch_norm,
                                                                                       reuse=reuse,
                                                                                       scope=end_point + deconv)
                        if j + 1 < sdc_num:
                            net = net_d
                            reuse = True
                        else:
                    ...
                    for j in range(sdc_num):
                        end_points[end_point] = net = slim.separable_conv2d(net, None, conv_def.kernel,
                                                                            depth_multiplier=1,
                                                                            stride=layer_stride,
                                                                            rate=layer_rate,
                                                                            normalizer_fn=slim.batch_norm,
                    ...
                    for j in range(sdc_num):
                        end_points[end_point] = net = slim.conv2d(net, depth(conv_def.depth), [1, 1],
                                                                  stride=1,
                                                                  normalizer_fn=slim.batch_norm,
                                                                  reuse=reuse,
                                                                  scope=end_point)
